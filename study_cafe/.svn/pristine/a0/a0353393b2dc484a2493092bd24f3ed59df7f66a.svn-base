...
<div id="reply-section" class="card pt-20" style="margin-top: 20px; padding: 0px 30px;">
	...
	<form:form commandName="replySGFormVO" action="list" method="post" id="replyForm">
		<div align="right" style="margin-top: 30px; ">
			...
			<form:hidden path="studygroupVO.memId"/>
		    
			<textarea rows="3" style="padding: 20p;" name="replyVO.replyContent" class="form-control" onclick="javascript:this.value='저도 참가하겠습니다. 카톡ID ddit4060'" placeholder="내용을 입력해주세요." ></textarea>
			...
			<form:hidden path="replyVO.studygroupNo" id="replyBoardNo" value="${sgv.studygroupNo }"/>
	...
	<div class="modal-dialog" role="document">
		<div class="modal-content">
			...
			<div class="modal-body">
				<textarea rows="3"   id="replyModifyContent" class="form-control" placeholder="내용을 입력해주세요." ></textarea>
...
function go_reply_page(pageNo){
	...
	go_action("<c:url value='/stb/detail'/>","#replyForm");
}

function replay_delete(replyNo){
	$("#deleteReplyNo").val(replyNo);

	go_action("<c:url value='/replySG/remove'/>","#replyForm");
}
...
function reply_regist(){
	...
	go_action("<c:url value='/replySG/regist'/>","#replyForm");
}
...
function reply_modify_view(replyNo){
	...
	$.ajax({
		url:"<c:url value='/replySG/modifyView' />",
		type:"post",
		data:data,
...
function modifyReply(){
	...
	go_action("<c:url value='/replySG/modify'/>","#replyForm");
