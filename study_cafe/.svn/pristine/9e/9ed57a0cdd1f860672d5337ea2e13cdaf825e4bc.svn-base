...
<section>
	<h1>발주내역 목록</h1>
	<form:form action="orderSuccess" commandName="orderListViewFormVO" id="orderListViewFrom" name="orderListViewFrom">
	<button class="icon-chevron-right btn btn-secondary">처리대기</button>
	
	<button class="btn btn-primary" type="button" style="float:right; display: inline;" onclick="fn_egov_selectList();" data-card-widget="search">
		<i class="fa fa-fw fa-search"></i>검색
	</button>
	<input type="text" placeholder="검색어를 입력하세요." id="searchOrderListViewVO.searchKeyword" name="searchOrderListViewVO.searchKeyword" value="${searchOrderListViewVO.searchKeyword }" style="float: right;display: inline;"/>
	<select name="searchOrderListViewVO.searchCondition" id="searchOrderListViewVO.searchCondition" style="float: right;display: inline;">
		<option value="p" ${searchOrderListViewVO.searchCondition eq 'p' ? 'selected':'' }>가맹점명</option>
	</select>
	<form:select path="searchOrderListViewVO.pageUnit" onchange="fn_egov_selectList()" style="float: right;display: inline;">
	...
	<table class="table table-hover mt-20">
		...
		<tbody>
			<c:if test="${empty refundWaitList}">
				<tr>
					<td colspan="7">
						<strong>해당 내용이 없습니다.</strong>
					</td>
			...
			<c:forEach items="${refundWaitList}" var="ref" varStatus="status">
			<c:if test="${ref.orderProductStatus eq 'OS05'}">
				<tr>
				  <td>
				  	<input type="checkbox" name="refundChk" value="${ref.orderProductNo}">
				  </td>
				  <td>${status.count }</td>
				  <td onclick="OpenWindow('orderDetail?orderProductNo=${ref.orderProductNo}', '발주목록 상세', 800, 600)">${oml.orderProductNo }</td>
				  <td>${ref.storeName}</td>
				  <td>${ref.productName }</td>
				  <td><fmt:formatNumber value="${ref.orderProductTotalPrice}" maxFractionDigits="3" /> 원</td>
				  <td>
				  	<fmt:formatDate value="${ref.orderProductRegDate}" pattern="yyyy-MM-dd"/>
				  </td>
				  <td>
	...
	<button class="btn btn-info icon-chevron-right" type="button" onclick="refundData()">반품처리</button>
		<div class="container clearfix" style="width: fit-content;">
			<ul class="pagination pagination-circle pagination-3d pagination-light">
				<ui:pagination paginationInfo = "${refundWaitpaginationInfo}" type="text" jsFunction="go_refundWait" />
				<form:hidden path="searchOrderListViewVO.pageIndex" id="refundWaitPageIndex"/>
			</ul>    
		</div>  
	</div>
	</form>
	...
	<table class="table table-hover mt-20">
		...
		<tbody>
			<c:if test="${empty refundSuccessList }">
				<tr>
					<td colspan="7">
						<strong>해당 내용이 없습니다.</strong>
					</td>
			...
			<c:forEach items="${refundSuccessList }" var="suc" varStatus="status">
			<c:if test="${suc.orderProductStatus eq 'OS06' }">
				<tr>
				  <td>${status.count }</td>
				  <td onclick="OpenWindow('orderDetail?orderProductNo=${suc.orderProductNo}', '발주목록 상세', 800, 600)">${suc.orderProductNo }</td>
				  <td>${suc.storeName}</td>
				  <td>${suc.productName }</td>
				  <td><fmt:formatNumber value="${suc.orderProductTotalPrice }" maxFractionDigits="3" /> 원</td>
				  <td>
				  	<fmt:formatDate value="${suc.orderProductRegDate }" pattern="yyyy-MM-dd"/>
				  </td>
				  <td>
		...
		<div class="container clearfix" style="width: fit-content;">
			<ul class="pagination pagination-circle pagination-3d pagination-light">
				<ui:pagination paginationInfo = "${refundSuccesspaginationInfo}" type="text" jsFunction="go_refundSuccess" />
				<form:hidden path="searchOrderListViewVO.pageIndex" id="refundSuccessPageIndex"/>
			</ul>    
		</div>  
	</div>
<!-- 	<div id="pagination" style="margin: auto;"> -->
