...
<div id="reply-section" class="card pt-20" style="margin-top: 20px; padding: 0px 30px;">
	...
	<form:form commandName="replyInfoFormVO" action="list" method="post" id="replyForm">
		<div align="right" style="margin-top: 30px; ">
			...
			<textarea rows="3" style="padding: 20p;" name="replyVO.replyContent" class="form-control" placeholder="내용을 입력해주세요." ></textarea>
			...
			<form:hidden path="replyVO.infoNo" id="replyBoardNo" value="${info.infoNo }"/>
...
function go_reply_page(pageNo){
	...
	go_action("<c:url value='/info/detail'/>","#replyForm");
}

function replay_delete(replyNo){
	$("#deleteReplyNo").val(replyNo);

	go_action("<c:url value='/replyInfo/remove'/>","#replyForm");
}
...
function reply_regist(){
	...
	go_action("<c:url value='/replyInfo/regist'/>","#replyForm");
}
...
function reply_modify_view(replyNo){
	...
	$.ajax({
		url:"<c:url value='/replyInfo/modifyView' />",
		type:"post",
		data:data,
...
function modifyReply(){
	...
	go_action("<c:url value='/replyInfo/modify'/>","#replyForm");
